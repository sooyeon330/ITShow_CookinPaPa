...
public class _1_bugerTestpanel2 extends JPanel{	
	...
	_1_bugerTestpanel2(JFrame frame) {
		...
		//버거패티
		JLabel basebuger = new JLabel(_1_bugerFrame.basebugimg);
		String bugername[] = {"buger2img","buger3img","buger4img","buger5img","buger7img","buger7img"};
		basebuger.setBounds(100, 530, _1_bugerFrame.basebugimg.getIconWidth(), _1_bugerFrame.basebugimg.getIconHeight());

		//성공 실패
		JLabel success = new JLabel(_1_bugerFrame.successimg);
		JLabel fail = new JLabel(_1_bugerFrame.failimg);
		success.setBounds(100, 100, _1_bugerFrame.successimg.getIconWidth(), _1_bugerFrame.successimg.getIconHeight());
		fail.setBounds(100, 100, _1_bugerFrame.successimg.getIconWidth(), _1_bugerFrame.successimg.getIconHeight());
		...
		//key이미지 셋팅
		for(int i=0; i<4; i++) {
			_1_bugerFrame.keyimg[i] = _1_bugerFrame.ImageResize(_1_bugerFrame.w, _1_bugerFrame.h,_1_bugerFrame.keyimg[i]);
			_1_bugerFrame.presskeyimg[i] = _1_bugerFrame.ImageResize(_1_bugerFrame.w, _1_bugerFrame.h,_1_bugerFrame.presskeyimg[i]);
		}
				...
				klisten2 = new KeyAdapter() {
						...
						public void keyPressed(KeyEvent e) {
							...
							if(e.getKeyCode() == 37||e.getKeyCode() == 38||e.getKeyCode() == 39||e.getKeyCode() == 40) {
									...
									keylb_2p[k2].setIcon(_1_bugerFrame.presskeyimg[keyarray2[0][k2]]); //라벨의 맞는 순서의 이미지를 가져옴
									
									if(e.getKeyCode() == keyarray2[1][k2++]) { //누른키와 눌러야하는 키가 같으면
										
										if(k2 > 3) { //인덱스 넘어가면
											twomenu.Score2 ++;
											bth = new _1_bugerThread(bugername[b++],panel,delbuger2);
											if(b > 5) {	
												success.setVisible(true);
												bth = new _1_bugerThread(bugername[4],panel,delbuger2);
												k2=0;b=0;
						...
						public void keyReleased(KeyEvent e) {
							keylb_2p[k2].setIcon( _1_bugerFrame.keyimg[keyarray2[0][k2]]);
							keylb_2p[k2].setIcon( _1_bugerFrame.keyimg[keyarray2[0][k2]]);
							keylb_2p[k2].setIcon( _1_bugerFrame.keyimg[keyarray2[0][k2]]);
							keylb_2p[k2].setIcon( _1_bugerFrame.keyimg[keyarray2[0][k2]]);
							frame.repaint();
	...
	synchronized protected void paintKey(JLabel[] keylb,int[][] array) { //새로운 키 배열 출력
		...
		for(int i=0; i<4; i++) { //랜덤으로 방향키 출력	
			...
			keylb[i]= new JLabel(_1_bugerFrame.keyimg[rand]); //라벨 배열에 순서대로 넣어주고
			keylb[i].setBounds(_1_bugerFrame.x+_1_bugerFrame.w*(i+1)+spacing, _1_bugerFrame.y, _1_bugerFrame.w, _1_bugerFrame.h); //위치지정
			
			add(keylb[i]); //add해줌 
			array[0][i] = rand; //랜덤값을 저장 -> 나중에 이미지 불러오기위해
			array[1][i] = _1_bugerFrame.keyCode(rand,"2");
